...
# Pre-train Attention -------------------------------
		...
		for epoch in range(pm.ATTENTION_PRE_TRAIN_EPOCH):
			...
			if epoch % 5 == 0:
				self.att_generate_samples(sess, attention_mechanism, pm.BATCH_SIZE, pm.GENERATED_NUM, pm.CN_PRE_GENERATOR_DATA, gen_data_loader)
...
# Pre-train Discriminator with positive datasets(real) and negative datasets from Generator model ---------------
		...
		for epoch in range(pm.D_PRE_TRAIN_EPOCH):
			self.att_generate_samples(sess, attention_mechanism, pm.BATCH_SIZE, pm.GENERATED_NUM, pm.CN_G_NEG_SAMPLING_DATA, gen_data_loader)
...
# Generator update(freezing Discriminator while updating Generator and Monte-Carlo Model one time per epoch) ----------
		...
		for total_batch in range(pm.TOTAL_BATCHES):
			...
			if total_batch % 5 == 0 or total_batch == pm.TOTAL_BATCHES - 1:
				self.att_generate_samples(sess, attention_mechanism, pm.BATCH_SIZE, pm.GENERATED_NUM, pm.CN_ADVERSARIAL_G_DATA, gen_data_loader)
...
# Discriminator update(freezing Generator while updating Discriminator five times per epoch) -------------
			...
			for i in range(pm.D_STEP):
				self.att_generate_samples(sess, attention_mechanism, pm.BATCH_SIZE, pm.GENERATED_NUM, pm.CN_ADVERSARIAL_NEG_DATA, gen_data_loader)
